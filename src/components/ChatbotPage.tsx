...
export default function ChatbotPage({ onNavigate }: ChatbotPageProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hi! I'm your UnlockGrowth AI assistant. I can help you understand lending documents, investment opportunities, and guide you through our platform. What would you like to know?",
      timestamp: new Date(),
      suggestions: [
        "Explain loan terms",
  ...
  // NEW: scroll only the messages panel, not the whole page
  ...
  const scrollToBottom = () => {
    ...
    if (!el) return;
    el.scrollTop = el.scrollHeight; // scrolls the panel, not the page
  };
  ...
  // Optional: stop browser from restoring prior scroll on back/forward
  useEffect(() => {
  ...
  useEffect(() => {
    // Stop the browser from restoring an old scroll position
    if ('scrollRestoration' in window.history) {
        window.history.scrollRestoration = 'manual';
    }
    // Land at the top when this page mounts
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    // Prevent page jump when focusing the input
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5">
      <div className="container px-4 py-8 max-w-6xl mx-auto">
        ...
        <Card className="shadow-xl">
          {/* Lock scrolling inside this card and prevent layout jump */}
          <CardContent className="p-0 flex flex-col h-[600px] md:h-[640px]">
            {/* Messages Area */}
            <div
              ref={messagesWrapRef}
